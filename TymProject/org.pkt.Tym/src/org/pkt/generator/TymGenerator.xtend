...
class TymGenerator extends AbstractGenerator {
			...
			public class Tym {
				...
				public static void main(String[] args) {
					«FOR s: resource.allContents.toIterable.filter(typeof(AbstractElement))»
						«IF (!(s.eContainer() instanceof Block) && !(s.eContainer() instanceof LoopStatement) && !(s.eContainer() instanceof FunctionBlock) && !(s.eContainer() instanceof Function))»
	...
	def interpretFunction(Function f) {
		...
		'''public static «IF (f.getReturn() == null)»void«ENDIF»«IF (f.getReturn() == "number")»int«ENDIF»«IF (f.getReturn() == "word")»String«ENDIF»«IF (f.getReturn() == "Bool")»Boolen«ENDIF» «f.getName()»(«IF (f.params.map[getVartype()].join(', ') == "number")»int«ENDIF»«IF (f.params.map[getVartype()].join(', ') == "Bool")»boolean«ENDIF»«IF (f.params.map[getVartype()].join(', ') == "word")»String«ENDIF»«  " " + f.params.map[ getName()].join(', ')») {
...
«ENDFOR»
		...
		switch (s) {
			Variable:
				if(s.expression instanceof Expression) {
					...
					switch(s.vartype) { 
						case "number":
							'''int «s.name» = Tym.«func.funcname.getName()»(«func.params.map[getNode().getText()].join(', ')»);'''
						case "word":
							'''String «s.name» = Tym.«func.funcname.getName()»(«func.params.map[getNode().getText()].join(', ')»);'''
						case "Bool":
							'''boolean «s.name» = Tym.«func.funcname.getName()»(«func.params.map[getNode().getText()].join(', ')»);'''
						default:
							'''«s.variable.name» = Tym.«func.funcname.getName()»(«func.params.map[getNode().getText()].join(', ')»);'''
					} 					
				}
			PrintStatement: {
				if(s.expression instanceof Expression) {
					...
					val func = s.expression as FunctionCall;
					'''System.out.println(Tym.«func.funcname.getName()»(«func.params.map[getNode().getText()].join(', ')»));'''
				}
			}
			Return:
			...
			FunctionCall:
				if(!(s.eContainer() instanceof PrintStatement) && !(s.eContainer() instanceof Variable)) { 
					'''tym.«s.funcname.getName()»(«s.params.map[getNode().getText()].join(', ')»);'''
				}
			TestStatement:
	...
	def private checkExpectedType(Expression exp, TymType expectedType) {
	...
	def private TymType getType(Expression exp) {
